if starting:
	...
	# Cyclic
	axis_x = VirtualAxis(axis_max, sensitivity=8, center_rate=0.015, use_falloff=True)
	axis_y = VirtualAxis(axis_max, sensitivity=8, center_rate=0.03, use_falloff=True)
	
	# Pedals
	axis_pedal = VirtualAxis(axis_max, sensitivity=200, center_rate=0.02, use_falloff=False)
	
	# Throttles (start at max)
	throttle_speed = VirtualAxis(1.0, sensitivity=0.025, center_rate=0.07)
	throttle_speed.set_trim(0.5)
	throttle_speed.set_val(0.5)
...
if keyboard.getKeyDown(Key.W):
	axis_throttle1.move(-1 * throttle_speed.value)
...
if mouse.wheel != 0 and not alt_pressed:
	axis_pedal.offset_trim(mouse.wheel * 5)
...
if not is_throttle_pressed:
	throttle_speed.apply_centering()
...
diagnostics.watch(throttle_speed.value)